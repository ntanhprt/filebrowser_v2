<template>
  ...
  <div v-else>
    <header-bar
      ...
      showMenu
      showLogo
      @logo-click="layoutStore.toggleSidebar()"
    />
...
import { name, baseURL } from "../utils/constants";
...
// Media viewer state
const showMediaViewer = ref<boolean>(false);
const mediaViewerItems = ref<any[]>([]);
const mediaViewerStartIndex = ref<number>(0);
...
const fetchData = async () => {
  ...
  try {
    ...
    // Remember folder path for navigation back
    if (res.isDir) {
      lastFolderPath.value = route.path;
    }

    // Auto-open MediaViewer for media files
    const mediaTypes = ['image', 'video', 'audio'];
    if (!res.isDir && mediaTypes.includes(res.type)) {
      openMediaViewer(res);
      // Navigate back to folder
      if (lastFolderPath.value) {
        router.push({ path: lastFolderPath.value });
      }
    }
...
// Get media URL for MediaViewer
const getMediaUrl = (item: any) => {
  return `${baseURL}/api/raw${item.path}`;
};

// Open MediaViewer for media files
const openMediaViewer = (file: any) => {
  // Get all media files from the folder listing
  const mediaTypes = ['image', 'video', 'audio'];
  
  if (!fileStore.oldReq?.items) return;
  
  const mediaFiles = fileStore.oldReq.items.filter((item: any) => 
    !item.isDir && mediaTypes.includes(item.type)
  );
  
  if (mediaFiles.length === 0) return;
  
  // Find index of clicked file
  let index = mediaFiles.findIndex((item: any) => item.path === file.path);
  if (index === -1) index = 0;
  
  mediaViewerItems.value = mediaFiles;
  mediaViewerStartIndex.value = index;
...
const closeMediaViewer = () => {
  ...
  // Navigate back to folder if needed
  if (lastFolderPath.value && route.path !== lastFolderPath.value) {
    router.push({ path: lastFolderPath.value });
  }
};
</script>
