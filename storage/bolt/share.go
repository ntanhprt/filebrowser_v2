...
func (s shareBackend) GetPublic() ([]*share.Link, error) {
	var v []*share.Link
	err := s.db.Select(q.Eq("IsPublic", true)).Find(&v)
	if errors.Is(err, storm.ErrNotFound) {
		return v, fberrors.ErrNotExist
	}
...
func (s shareBackend) GetPublicByUserID(id uint) ([]*share.Link, error) {
	var v []*share.Link
	err := s.db.Select(q.Eq("IsPublic", true), q.Eq("UserID", id)).Find(&v)
	if errors.Is(err, storm.ErrNotFound) {
		return v, fberrors.ErrNotExist
	}
